...
const AlbumManagement = () => {
  ...
  const fetchData = async () => {
    await AxiosInterceptors.get(urlConfig.albums.getAllAlbums + `?limit=10&page=${pageCount}`)
      .then((res) => {
        if (res && res.status === 200) {
          if (res.data.pagination.albums) {
            setAlbum(res.data.pagination.albums)
  ...
  return (
    ...
    >
      <Helmet>
        <title>{t('albumManagement')}</title>
      </Helmet>
      {isLoading ? (
        <Loading />
      ) : album.length > 0 ? (
        <AlbumTable majorsOrder={album} fetchData={fetchData} pageCount={pageCount} setPageCount={setPageCount} />
      ) : (
